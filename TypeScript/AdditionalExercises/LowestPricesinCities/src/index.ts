...
type TownPriceMap = { [townName: string]: number };
type ResultType = { [key: string]: TownPriceMap };
...
function solve(input: string[]): void {
    ...
    for (const line of input) {
        ...
        if (!result[product][town] || result[product][town] > currentPrice) result[product][town] = currentPrice;
    }
    Object.entries(result).forEach(([product, townsWithPriceMap]) => {
        const [town, price] = Object.entries(townsWithPriceMap).sort((a, b) => a[1] - b[1])[0];
